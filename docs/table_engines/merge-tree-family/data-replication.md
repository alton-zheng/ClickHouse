...
副本不依赖`shard`。每个`shard`有它自己的独立副本。
...
`SELECT`  查询并不需要借助 ZooKeeper ，复本并不影响  `SELECT`  的性能，查询复制表与非复制表速度是一样的。查询分布式表时，ClickHouse 的处理方式可通过设置  [max_replica_delay_for_distributed_queries]()  和  [fallback_to_stale_replicas_for_distributed_queries]()  修改。
...
对于非常大的集群，你可以把不同的 ZooKeeper 集群用于不同的`shard`。然而，即使 Yandex.Metrica 集群（大约 300 台服务器）也证明还不需要这么做。
...
“ZooKeeper 中该表的路径”对每个可复制表都要是唯一的。不同`shard`上的表要有不同的路径。 这种情况下，路径包含下面这些部分：

`/clickhouse/tables/`  是公共前缀，我们推荐使用这个。

`{layer}-{shard}`  是`shard`标识部分。在此示例中，由于 Yandex.Metrica 集群使用了两级`shard`，所以它是由两部分组成的。但对于大多数情况来说，你只需保留 {shard} 占位符即可，它会替换展开为`shard`标识。
...
副本名称用于标识同一个表`shard`的不同副本。你可以使用服务器名称，如上例所示。同个`shard`中不同副本的副本名称要唯一。
...
如果本地数据集与预期数据的差异太大，则会触发安全机制。服务器在日志中记录此内容并拒绝启动。这种情况很可能是配置错误，例如，一个`shard`上的副本意外配置为别的`shard`上的副本。然而，此机制的阈值设置得相当低，在正常故障恢复期间可能会出现这种情况。在这种情况下，数据恢复则是半自动模式，通过用户主动操作触发。
